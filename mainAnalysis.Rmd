...
This notebook is extension to the jupiter notebook that extracts the results of FreeSurfer and TRACULA analysis
...
```{r}
unc_ANOVA <- lmer(FA ~ med_cond * Ses + gender + age + TMI + (1|Sub), data = WM)

print(anova(unc_ANOVA))
print(confint(unc_ANOVA))
```
...
Significant reduction in FA in the Ketamine group between session 1 and session 2 that persisted into the follow up

```{r}
groups <- c(1, 0)

for (group in groups){
  drug <- subset(WM, med_cond == group)
  drugANOVA <- lmer(FA ~ Ses + gender + age + TMI + (1|Sub), data = drug)
  
  print(drug$DrugN[1])
  print(anova(drugANOVA))
  print(confint(drugANOVA))
  print(emmeans(drugANOVA, list(pairwise ~ Ses), adjust = "tukey"))
}
```
...
```{r, include=FALSE}
#bModel <- brm(FA ~ med_cond * Ses + gender + age + TMI + (1|Sub), data = WM, 
#              iter = 4000, warmup = 500, chains = 8, cores = 8, seed = 1985)

#bModel_p <- brm(FA ~ med_cond * Ses + gender + age + TMI + (1|Sub), data = WM, family = gaussian(), prior = set_prior('normal(0, 3)'),
#              iter = 4000, warmup = 500, chains = 8, cores = 8, seed = 1985)
```

```{r, include=FALSE}
bModel_p <- brm(FA ~ med_cond * Ses + gender + age + TMI + (1|Sub), data = WM, family = student(), prior = set_prior('normal(0, 1)'),
              iter = 2000, warmup = 1000, chains = 4, cores = 4, seed = 2019)
```

```{r}
#print(summary(bModel, prob = .89) %>% print(digits = 4))
